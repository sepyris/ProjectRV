...
public class BashDash : ActiveSkillBase
{
    ...
    private const float TARGET_SEARCH_RADIUS = 3f;  // 적 탐색 반경
    private const float DASH_DISTANCE = 3f;          // 대쉬 거리
    ...
    public override string DetailDescription
    {
        get
        {
            return $"{Description}\n\n{TARGET_SEARCH_RADIUS}범위내에 적이 있다면 자동으로 적에게 돌진하여 \n 공격력의 {GetCurrentDamage()}% 만큼의 데미지를 줍니다.\n범위내에 적이 없다면 바라보는 방향으로 {DASH_DISTANCE}거리만큼 이동 합니다.";
        }
    }
